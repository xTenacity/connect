...
export default class AIPlayer {
    ...
    private boardKey(board: Board, depth: number, maximizing: boolean) {
    ...
    evaluate(board: Board, winner: string, depthLeft: number): number {
        if (winner === this.aiPiece) return 1000 + depthLeft;
        if (winner === this.playerPiece) return -1000 - depthLeft;
        let score = 0;
        // simple heuristic: prefer center columns
        const center = Math.floor(board.width / 2);
        for (let r = 0; r < board.height; r++) {
            if (board.getAt(r, center) === this.aiPiece) score += 5;
            if (board.getAt(r, center) === this.playerPiece) score -= 5;
        }
        return score;
    }
}
